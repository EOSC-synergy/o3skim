...
def load_tco3(model_path):
    """Loads and returns a CCMI-1 U-CAMBRIDGE DataArray model and the dataset
    attributes.
    ...
    logger.info("Loading CCMI-1 U-CAMBRIDGE data from: %s", model_path)
    ...
    dataset[settings.TCO3_VAR].attrs.update(
        {"standard_name": settings.TCO3_STANDARD_NAME}
    )
